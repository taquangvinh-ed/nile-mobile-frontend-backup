...
export default function ProductDetails() {
  ...
  const { productDetails, loading, error, isAuthenticated, cart, cartLoading, cartError, reviews, reviewLoading, reviewError } = useSelector((state) => state.auth);
  ...
  useEffect(() => {
    ...
    if (productDetails && productDetails.variations && productDetails.variations.length > 0 && !selectedVariation) {
      const firstValidVariation = productDetails.variations[0];
      setSelectedVariation({
        ...firstValidVariation,
        variationId: Number(firstValidVariation.variationId),
      });
  ...
  useEffect(() => {
    if (selectedVariation && selectedVariation.variationId) {
      const variationId = Number(selectedVariation.variationId);
      console.log("Dispatching getReviewsByVariation with variationId:", variationId);
      dispatch(getReviewsByVariation(variationId));
    } else {
      console.warn("No valid selectedVariation for reviews:", selectedVariation);
    }
  ...
  const handleThumbnailClick = (index) => {
    ...
    if (newVariation && newVariation.variationId) {
      setSelectedVariation({
        ...newVariation,
        variationId: Number(newVariation.variationId),
      });
  ...
  const handleAddToCart = async (event) => {
    ...
    try {
      const variationWithNumberId = {
        ...selectedVariation,
        id: Number(selectedVariation.variationId),
      };
  ...
  const handleBuyNow = async (event) => {
    ...
    if (!selectedVariation || !selectedVariation.variationId) {
      alert("Vui lòng chọn một biến thể hợp lệ trước khi mua ngay.");
      return;
    }

    try {
      const variationId = Number(selectedVariation.variationId);
      const result = await dispatch(buyNow(variationId));
      if (result.payload.success) {
        // Làm mới dữ liệu giỏ hàng
        await dispatch(getCart());
        navigate("/cart");
      } else {
        alert("Không thể xử lý mua ngay: " + result.payload.error);
      }
    } catch (err) {
      alert("Đã xảy ra lỗi: " + (cartError || "Không xác định"));
    }
    ...
    <div className="bg-white">
      <div className="pt-6">
        ...
        <section className="mx-75 grid grid-cols-1 lg:grid-cols-2 gap-x-3 gap-y-10 pr-4 pt-10">
          ...
          <div className="flex flex-col items-center w-[40rem] h-[30rem]">
            <div className="overflow-hidden rounded-lg max-w-[40rem] max-h-[25rem] mb-4">
              <img alt={selectedVariation?.imageURL ? "Product Image" : "No Image"} src={selectedVariation?.imageURL || productDetails.imageURL || ""} className="w-full h-full object-cover" />
            </div>
            <div className="flex space-x-2 justify-center overflow-x-auto">
              {productDetails.variations.map((variation, index) => (
                <div
                  key={variation.variationId}
                  className={`aspect-h-1 aspect-w-1 overflow-hidden rounded-lg max-w-[4rem] max-h-[4rem] cursor-pointer ${index === currentImageIndex ? "border-2 border-blue-500" : ""}`}
                  onClick={() => handleThumbnailClick(index)}
                >
                  <img alt={variation.imageURL ? "Variation Image" : "No Image"} src={variation.imageURL || ""} className="w-full h-full object-cover" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="border shadow-2xl lg:col-span-1 max-auto max-w-2xl px-2 pb-16 sm:px-6 lg:max-w-7xl lg:px-8 ml-20">
            <div className="lg:col-span-2 lg:border-r lg:border-gray-200 lg:pr-8">
              <h1 className="text-lg lg:text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">{productDetails.name}</h1>
            </div>

            <div className="mt-4 lg:row-span-3 lg:mt-0">
              <h2 className="sr-only">Product information</h2>
              <div className="text-3xl tracking-tight text-gray-900">
                <ProductPrice originalPrice={formatedPrice} discountPrice={formatedDiscountPrice} savings={selectedVariation?.discountPercent} />
              </div>
              ...
              <form className="mt-10">
                      ...
                      {productDetails.variations.map((variation) => (
                        ...
                        >
                          <div className="flex justify-center items-center mx-2">
                            <span aria-hidden="true" className={`size-8 rounded-full border border-black/10`} style={{ backgroundColor: variation.color }} />
                            <span className="mx-1">
                              {variation.color} <br />
                ...
                <div className="flex flex-col my-8">
                  ...
                  >
                    {cartLoading ? "Đang thêm..." : "Thêm vào giỏ hàng"} <AddShoppingCartIcon />
                  </button>
                </div>
              </form>
            </div>
                ...
                <div className="mt-4">
                  <ul role="list" className="list-disc space-y-2 pl-4 text-sm">
                    <li className="text-gray-400">
                      <span className="text-gray-600">Kích thước màn hình: {productDetails.screenSize}"</span>
                    </li>
                    <li className="text-gray-400">
                      <span className="text-gray-600">Dung lượng pin: {productDetails.batteryCapacity} mAh</span>
                    </li>
                    <li className="text-gray-400">
                      <span className="text-gray-600">Hệ điều hành: {productDetails.os}</span>
                    </li>
                  </ul>
                  <div>
                    <ProductDetailPopup product={productDetails} variation={selectedVariation} />
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="mx-75 mt-10 flex flex-col items-start">
          <h1 className="text-2xl font-semibold">Đánh giá {productDetails.name}</h1>
          <div className="border p-5 rounded-lg shadow-2xl">
            <div className="grid grid-cols-12 gap-7">
              <div className="col-span-7">
                {selectedVariation && selectedVariation.variationId ? (
                  <InputPersionalnfor rating={true} variationId={selectedVariation.variationId} />
                ) : (
              ...
              <div className="col-span-7">
                <div className="space-y-5">
                  {reviewLoading ? (
                    ...
                    <div>
                      <Typography>Hiện tại không có review cho sản phẩm này !!!</Typography>
                    </div>
                  )}
                </div>
              </div>
